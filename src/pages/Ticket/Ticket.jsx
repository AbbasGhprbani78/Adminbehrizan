...
export default function Ticket() {
  ...
  const handleTicketCloseChange = (ticketId, isChecked) => {
    ...
    const newOpenTicketCount = allTickets.filter(ticket => ticket.ticket_close === false).length;
  ...
  const sendmessage = async () => {
    ...
    if (textInput.trim()) {
      ...
      try {
        const response = await axios.post(`${apiUrl}/chat/send-ticket/`, formData, {
          headers,
        })
  ...
  useEffect(() => {
    const allOpenTicket = allTickets?.filter(ticket => ticket.ticket_close == false)
        ...
        <div className={styles.maincontent}>
          {
            windowWidth < 1025 ?
              <>
                ...
                <div className={`${tab === 3 ? styles.TicketMassageBox : styles.noneBox}`}>
...

                  {
                    ticket?.ticket_close ?
                      <>
                        <div className={`${styles.wrapinpt_m_close} ${styles.close_ticket} d-flex align-items-center text-center`}>
              ...
              <>
                <div className={styles.ButtonBox}>
                  <div className={`${styles.Button1} ${styles.activetab}`} onClick={() => {
                    getAllTicket()
                    setTab(1)
                  }}>
                    <span>تیکت ها</span>
                ...
                <div className={`${tab === 3 ? styles.TicketMassageBox : styles.noneBox}`}  >
                  ...
                  </div>
                  {
                    ticket?.ticket_close ?
                      <>
                        <div className={`${styles.input_message_p}  ${styles.close_ticket} d-flex align-items-center`}>
                          <span >تیکت بسته شد</span>
                          <CiLock className={styles.lockicon} />
                        </div>
                      </> :
